...
class QNN_0_Geometric_Sz(nn.Module):

    def __init__(self, num_layers, num_features, init_weights=None):
        super(QNN_0_Geometric_Sz, self).__init__()
        ...
        self.weights = nn.ParameterList([
            nn.Parameter(init_weights[i] if init_weights else torch.rand(8) * 2 - 1)
    ...
    def forward(self, batch):
        ...
        Sz_batch = self.Sz.unsqueeze(0).expand(B, -1, -1)
        exp_Sz = torch.real(torch.einsum("bij,bij->b", rho, Sz_batch))
